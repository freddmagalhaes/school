...
const KpiCard = ({ titulo, valor, sub, icon: Icon, cor, tendencia }: KpiCardProps) => {
  const c = COR_MAP[cor];
  return (
    <div className={`bg-[#0e1425] border ${c.border} rounded-2xl p-5 hover:border-opacity-50 transition-all`}>
      <div className="flex items-start justify-between mb-4">
        <div className={`w-10 h-10 ${c.bg} border ${c.border} rounded-xl flex items-center justify-center`}>
          <Icon size={18} className={c.icon} />
        </div>
        {tendencia !== undefined && (
          <div className={`flex items-center gap-1 text-xs font-semibold ${tendencia >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-amber-400 bg-amber-400/10 border border-amber-400/20 px-3 py-1.5 rounded-full font-semibold">
          <BarChart2 size={12} />
          Dados ao vivo
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-[#0e1425] border border-[#1e2d4a] rounded-2xl p-5">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-bold text-white text-sm">Receita Mensal (MRR)</h2>
            <span className="text-[10px] text-gray-500 uppercase tracking-wider">Últimos 6 meses</span>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={RECEITA_MOCK} barSize={28}>
              ...
              <Tooltip
                contentStyle={{ background: '#141c2e', border: '1px solid #1e2d4a', borderRadius: 12, fontSize: 12 }}
        ...
        <div className="bg-[#0e1425] border border-[#1e2d4a] rounded-2xl p-5">
          <h2 className="font-bold text-white text-sm mb-5">Distribuição por Plano</h2>
      ...
      <div className="bg-[#0e1425] border border-[#1e2d4a] rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-white text-sm">Aguardando Ativação</h2>
          </div>
        ...
        {pendentes.length === 0 && !loading ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 text-sm">Nenhuma assinatura pendente!</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#1e2d4a]">
                {(pendentes.length > 0 ? pendentes : [
                  { id: '1', nome: 'João Silva', nome_escola: 'E.E. José de Alencar', plano: 'profissional', ciclo: 'mensal', valor_total: 397, status: 'pendente', created_at: '2026-04-20' },
                  { id: '2', nome: 'Maria Santos', nome_escola: 'Colégio Boa Vista', plano: 'basico', ciclo: 'anual', valor_total: 1970, status: 'pendente', created_at: '2026-04-19' },
                  { id: '3', nome: 'Pedro Costa', nome_escola: 'Instituto Nova Geração', plano: 'enterprise', ciclo: 'anual', valor_total: 7970, status: 'pendente', created_at: '2026-04-18' },
                ] as Assinatura[]).map((a) => (
                  <tr key={a.id} className="hover:bg-white/2 transition-colors">
                    <td className="py-3 text-gray-200 font-medium">{a.nome}</td>
                    <td className="py-3 text-gray-400">{a.nome_escola}</td>
                    <td className="py-3">
                    ...
                    <td className="py-3 text-gray-200">R$ {a.valor_total.toLocaleString('pt-BR')}</td>
